...
tail -50 /dev/shm/paperclip.log`}</CodeBlock>
        ...
        <Warn>
          <strong>Use <InlineCode>claude setup-token</InlineCode>, not <InlineCode>claude login</InlineCode>.</strong> The browser-based <InlineCode>claude login</InlineCode> generates short-lived tokens that expire every ~6 hours — completely unsuitable for a 24/7 server. <InlineCode>setup-token</InlineCode> generates a long-lived OAuth token designed for headless environments like Zo.
        </Warn>
        <P>
          Run this on a machine where you have a browser (e.g., your laptop):
        </P>
        <CodeBlock title="bash — run on your laptop">{`claude setup-token`}</CodeBlock>
        <P>
          This will authenticate via browser and output a long-lived <InlineCode>sk-ant-oat01-</InlineCode> token. Copy it.
        </P>
        <P>
          Then on your Zo Computer, create a user and set up the credentials:
        </P>
        <CodeBlock title="bash — run on Zo Computer">{`# Create a user if you don't have one
...
EOF
chown -R zoey:zoey /home/zoey/.claude`}</CodeBlock>
        <Info>
          The <InlineCode>setup-token</InlineCode> command requires an interactive terminal with a browser, so it cannot be run directly on the Zo Computer. Generate the token locally and transfer it. The token is long-lived — you won't need to repeat this regularly.
        </Info>
...
# Expected output: hello`}</CodeBlock>
        <P>If you see "hello" (or similar), Claude Code is working as the postgres user. If you get auth errors, verify your <InlineCode>setup-token</InlineCode> is correct and the credentials files were copied properly.</P>
